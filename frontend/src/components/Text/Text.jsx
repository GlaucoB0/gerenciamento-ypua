...
const Text = ({ type = 'Paragraph', color = '#353535', fontFamily = 'normal', children }) => {
    let styles = getStyles(color, fontFamily)

    switch (type) {
        case 'GiantText':
            return <h1 className={$.giantText} style={styles}>{children}</h1>
        case 'BigText':
            return <h1 className={$.bigText} style={styles}>{children}</h1>
